...
class _ExerciseTemplateState extends State<ExerciseTemplate> with TickerProviderStateMixin {
      ...
      MaterialPageRoute(
        builder: (context) => CompletionScreen(
          ...
          onContinue: () {
            ...
            Navigator.of(context).pop(); // 운동 화면 닫기
  ...
  CompletionType _determineCompletionType() {
    final questions = widget.exerciseData['questions'] ?? [];
    if (questions.isEmpty) {
      return CompletionType.great; // 기본값
    }

    final totalQuestions = questions.length;
    final correctRate = _correctAnswers / totalQuestions;
    
    if (correctRate >= 0.9) {
      return CompletionType.perfect; // 90% 이상 정답
    } else if (correctRate >= 0.7) {
      return CompletionType.great; // 70% 이상 정답
    } else if (correctRate >= 0.5) {
      return CompletionType.encourage; // 50% 이상 노력
    } else {
      return CompletionType.enough; // 많이 틀렸지만 충분히 노력함
    }
  }
}
